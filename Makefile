...
$(TESTBINDIR)/$(SPEED_TEST_NAME): $(MAP_ROUTER_OBJ) $(SPEED_TEST_OBJ) $(CSV_WRITER_OBJ) $(XML_READER_OBJ) $(STRING_UTILS_OBJ) $(LIBDIR)/$(LIBCSV_NAME)
	$(CXX) $(MAP_ROUTER_OBJ) $(SPEED_TEST_OBJ) $(CSV_WRITER_OBJ) $(XML_READER_OBJ) $(STRING_UTILS_OBJ) /usr/local/lib/$(LIBCSV_NAME) $(CXXFLAGS) $(TESTLDFLAGS) -o $(TESTBINDIR)/$(SPEED_TEST_NAME)

#testrouter.cpp
$(TEST_ROUTER_OBJ): $(SRCDIR)/testrouter.cpp $(INCLUDEDIR)/MapRouter.h
	$(CXX) $(SRCDIR)/testrouter.cpp -c $(CXXFLAGS) -o $(TEST_ROUTER_OBJ)

$(TESTBINDIR)/$(TEST_ROUTER_NAME): $(MAP_ROUTER_OBJ) $(TEST_ROUTER_OBJ) $(XML_READER_OBJ)
	$(CXX) $(MAP_ROUTER_OBJ) $(TEST_ROUTER_OBJ) $(XML_READER_OBJ) $(CXXFLAGS) $(TESTLDFLAGS) -o $(TESTBINDIR)/$(TEST_ROUTER_NAME)

$(MAIN_OBJ): $(SRCDIR)/main.cpp
	$(CXX) $(SRCDIR)/main.cpp -c $(CXXFLAGS) -o $(MAIN_OBJ)

$(TESTBINDIR)/$(MAIN_NAME): $(MAIN_OBJ) $(CSV_READER_OBJ) $(XML_READER_OBJ) $(MAP_ROUTER_OBJ) $(LIBDIR)/$(LIBCSV_NAME)
	$(CXX) $(MAIN_OBJ) $(CSV_READER_OBJ) $(XML_READER_OBJ) $(MAP_ROUTER_OBJ) $(CXXFLAGS) $(LDFLAGS) -o $(TESTBINDIR)/$(MAIN_NAME)
